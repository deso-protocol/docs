...
**Starting today, and ending on the launch date of NFTs \(TBD\), the core dev team will be accepting NFT submissions from all users to participate in bitclout.com's first major NFT drop!** Any member of the community can submit an NFT using [this form](https://tbd.com), and the winners will be the first ones ever to be featured on bitclout.com's NFTs tab. Once the NFTs are up, all users will then be able to bid on them, and potentially win them!
